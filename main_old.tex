% !TEX program = latexmk
% !TEX encoding = UTF-8 Unicode

% 注意：请使用 TeXLive 2019 及以上版本，已在 2019 2020 2021 版本测试编译成功，已在 2018 测试编译失败
% 如不想更新，可尝试使用 手动编译 XeLaTeX（可能出错）
% WARNING: Please use TeXLive version >= 2019, This template has been tested and found available on 2019, 2020, 2021, unavailable on 2018. 

% 注意：请确保自己已经完整阅读了 README.md 这一 markdown 文档，部分功能的使用方法并未在 .tex 中直接给出；如果仍有使用问题，请在 Github 上提出 issue
% WARNING: Pleans read README.md first cause some usages are not given in .tex files.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% select the basic style of this thesis/dissertation (this document will use thesis for convenience)
%% 选择论文的基本类型

\documentclass[
    master,     % 必选项：   {master, doctor} 此处不区分专业/学术学位，在下面学科类型处区分
                % Mandatory: {master, doctor} No difference between Academic degree and Professional degree,
                %                             but be careful about arguments in `\degree' and `\subject'
    % english,  % 可选项：   英文正文请选择此项
                % Optional:  For english main content, It will change some auto-generated matter into English
    % blind,    % 可选项：   论文用于盲审请选择此项，将不会产生致谢(内容)、答辩委员会会议决议(内容)、
                %            常规评阅人名单(内容)，注意请自行修改 `攻读学位期间取得的研究成果' 内的内容格式
                % Optional:  For blind review, It will not generate Acknowledgements(Content), Decision of Defense Committee(Content),
                %            General Reviewers List(Content), CHANGE THE STYLE of Achievements BY YOURSELF
]{XJTU-thesis}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% fill the each blank for auto-generate contents
%% 填写以下信息用以自动生成

% 论文标题（不超过35个字，英文注意大小写规律）
% Title, Make sure you have an acceptable capitalization
\title{基于WiFi信号的类增量动作识别方法研究}{Research on Class Incremental Action Recognition Methods Based on WiFi Signals}

% 学位类型
% Type of your degree, Translate it from documents in `Materials/Requirements/2021/01 中英文题名页示例/英文标准翻译/'
%\degree{硕士}{Engineering} % 学术型(Academic)硕士请基于 '学术学位名称.txt' 填写
\degree{硕士}{Engineering} % 专业型(Professional)硕士请基于 '专业学位（领域）英文标准翻译.pdf' 填写
%\degree{博士}{Philosophy}  % 学术型(Academic Doc)博士请填写 '{博士}{Philosophy}'
%\degree{博士}{Engineering} % 专业型(Professional Doc)博士请基于 '专业学位（领域）英文标准翻译.pdf' 填写

% 作者姓名（注意：所有人名英文均为「名在前，姓在后」，如果只有外文名，请在两个参数都填写外文名称）
% If you have only foreign name, put it as both first and second argument
\author{罗弘林}{Honglin Luo}
% \author{}

% 指导老师姓名（注意：同作者姓名）
% Name of supervisor, It have the same requirments as the author
\advisor{王飞}{副教授}{Fei Wang}{Assoc. Prof.}
% \advisor{}
% 合作指导老师姓名 或 老师团队（合作指导老师指：1.与招生简章中一致的合作导师，2.CSC项目的合作导师）
%（校方模板要求只能选择一个，都有则显示合作导师）
% Name of associate advisor or adviror's team, You can use only one of them, and advisorassociate has a higher priority.
% \advisorassociate{陈尘}{副教授}{Chen Chen}{Asscociate Prof.}
% \advisorteam{团队中文名}{English Name of the Team}

% 封面显示为「类别（领域）：…」，取值见 \subject 第一参数（与校方模板一致时可配合下方 \patchcmd）
% 英文题名页仍输出第二参数，请与院系要求一致
\subject{工程硕士}{Master of Engineering}
% \subject{}

% 答辩委员会委员 显示的顺序和这里的一样 第一个人是主席
% Committee member of your defence, notice that the order shows in the thesis is same as here, and the first one is the chairperson
% each member should be put as {Organization,Name,Title} split by comma
\addcommitteemember{西安交通大学教授,李垚辰}
\addcommitteemember{西安交通大学副教授,金莉}
\addcommitteemember{西安交通大学副教授,杜小智}
\addcommitteemember{西安交通大学副教授,崔琼杰}
\addcommitteemember{西安交通大学助理教授,董永昊}

% 答辩时间（手动指定）
% Defence date, input manually
\defensedate{2026}{05}{08}

% 答辩地点（涉密论文请手动设置为「西安交通大学」）
% Defence location, default value is 「西安交通大学」
\defenseloc{西安交通大学创新港4-3218}

% 论文提交日期，不输入参数则默认使用当前日期，如手动指明年月，请在第一个可选参数内填写年份，第二个可选参数填写月份（均为阿拉伯数字）
% Submission date of this thesis, if you not put it manually, it will use the current time
% \submitdate[2021][06]
\submitdate

% 常规审阅人 要求和答辩委员会委员一样
% General reviewer list, same requirements as the committee member
\addgeneralreviewer{}
\addgeneralreviewer{}
\addgeneralreviewer{}
\addgeneralreviewer{}
\addgeneralreviewer{}

% 学院
% School or Faculty, unused now
% \school{电气工程学院}{School of Electrical Engineering}

% 专业[学士学位使用]
% Major, unused now
% \stumajor{计算机科学与计数}{Computer Science}

% 学号[学士学位使用]
% Student ID, unused now
% \stuid{}

% 班级[学士学位使用]
% Administrative class, unused now
% \adminclass{电气7xx班}{}

\addbibresource{References/reference.bib}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 如果有使用其他包，请在这里添加
%% If you need other packages, use them here

\usepackage{xurl}
% 改善参考文献中 DOI/URL 在 PDF 中的断行位置，降低被误读为截断编号的风险
\makeatletter
\g@addto@macro\UrlBreaks{\do\-\do\/\do\.\do\_}
\makeatother

\usepackage{placeins}

% 盲审意见16：图、表、公式与正文之间间距统一为约 0.5 倍行距（随正文字号与行距缩放）
% \AtBeginDocument{%
%   \setlength{\textfloatsep}{0.5\baselineskip}%
%   \setlength{\floatsep}{0.5\baselineskip}%
%   \setlength{\intextsep}{0.5\baselineskip}%
%   \setlength{\abovedisplayskip}{0.5\baselineskip}%
%   \setlength{\belowdisplayskip}{0.5\baselineskip}%
%   \setlength{\abovedisplayshortskip}{0.5\baselineskip}%
%   \setlength{\belowdisplayshortskip}{0.5\baselineskip}%
% }

% 盲审意见17：在进入/离开 figure 时临时收紧 \intextsep、\textfloatsep、\floatsep，压缩图题至正文/小节标题的留白。
% table 与 equation 不受影响；公式上下间距仍仅由上方盲审16段落控制。若插图被推迟到页顶/页底排出，仍以全局 0.5\baselineskip 为准；文中 [H] 等就地排出的图仍适用本收紧。
\newlength{\xjtufigskipSaveIntext}
\newlength{\xjtufigskipSaveTextfloat}
\newlength{\xjtufigskipSaveFloatsep}
\AtBeginEnvironment{figure}{%
  \setlength{\xjtufigskipSaveIntext}{\intextsep}%
  \setlength{\xjtufigskipSaveTextfloat}{\textfloatsep}%
  \setlength{\xjtufigskipSaveFloatsep}{\floatsep}%
  \setlength{\intextsep}{0.35\baselineskip}%
  \setlength{\textfloatsep}{0.35\baselineskip}%
  \setlength{\floatsep}{0.35\baselineskip}%
}
\AtEndEnvironment{figure}{%
  \setlength{\intextsep}{\xjtufigskipSaveIntext}%
  \setlength{\textfloatsep}{\xjtufigskipSaveTextfloat}%
  \setlength{\floatsep}{\xjtufigskipSaveFloatsep}%
}

% 将中文题名页固定标签「学科名称：」改为「类别（领域）：」（模板定义在 XJTU-thesis.cls 的 \thesistitles 中）
\makeatletter
\patchcmd{\thesistitles}{学科名称：}{类别（领域）：}{}{%
  \PackageWarning{xjtu-thesis-custom}{未能替换题名页「学科名称」标签，请检查 XJTU-thesis.cls 是否与模板一致。}%
}
\makeatother

% 参考文献：按 GB/T 7714 顺序编码制盲审要求，仅列出正文引用文献（与 cls 一致使用 keyword=references）
\makeatletter
\RenewDocumentCommand {\thesisbibliography} { o } {
  \renewcommand{\thesisbibname}{参考文献}
  \ifthesis@english
    \renewcommand{\thesisbibname}{References}
  \fi
  \pdfbookmark{\thesisbibname}{References}
  \hypersetup{bookmarksdepth=-2}
  \addcontentsline{toc}{chapter}{参考文献}
  \addcontentsline{toe}{chapter}{References}
  \hypersetup{bookmarksdepth=2}
  \printbibliography[heading=bibliography,title=\thesisbibname,keyword={references},resetnumbers=true]
  \pagecheck

  \IfNoValueTF {#1} {
  }{
    \newpage
    \edef\hmm{\pdfpagewidth=\the\pdfpagewidth \pdfpageheight=\the\pdfpageheight\relax}
    \pdfpagewidth=80cm
    \pdfpageheight=130cm
    \newgeometry{top=1in,left=1in,textwidth=70cm,textheight=120cm}
    \printbibliography[heading=bibliography,title=\thesisbibname,keyword={references},resetnumbers=true]
    \restoregeometry
    \hmm
  }
}
\makeatother


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 注意：根据校方要求，以下所有页面顺序不可调整
%% Notice: The order of these pages are defined in the requirements of the University.

\begin{document}
% [自动生成] 中英题名页
% [Auto Generate] Chinese English Title Page
\thesistitles

% [自动生成] 答辩委员会页
\thesiscommittes

% 生成摘要页 修改 Main_Miscellaneous/abstract_chs/eng.tex 中的内容
% Abstract, Rewrite your content in Main_Miscellaneous/abstract_chs/eng.tex
\thesisabstract

% 中英目录页
% Table of Contents
\thesistableofcontens

% 主要符号表（已删除）
% \thesisglossarylist

% 正文 注意：英文正文写作时，中、英标题还是先中后英标题；同时，下面参数的顺序有意义，不要乱放
% Main contents, X of cX is the chapter of the thesis, you can change it if you want, the order matters
% and KEEP Chinese Title as the FIRST argument, English Titile as the SECOND, if you use `english' option

\thesisbody{
    Main_Spine/c1,
    Main_Spine/c2,
    Main_Spine/c3,
    Main_Spine/c4,
    Main_Spine/c5
}

% 致谢 修改 Main_Miscellaneous/acknowledegment.tex 中的内容
% Acknowledgement, Rewrite your content in Main_Miscellaneous/acknowledegment.tex
\thesisacknowledegment

% [自动生成] 参考文献 默认使用 References/reference.bib，手动指定请在导言区 \addbibresource 处指定
% [Auto Generate] Bibliography, Default file is References/reference.bib, change argument in \addbibresource for manual specification
\thesisbibliography

% 附录（已删除）
% \thesisappendix{Main_Miscellaneous/appendix_a,
%                 Main_Miscellaneous/appendix_b}

% 攻读学位期间取得的研究成果 修改 Main_Miscellaneous/achievement.tex 中的内容 注意盲审时需要手动修改格式
\thesisachivements

% 答辩委员会决议 修改 Main_Miscellaneous/decision.tex 中的内容
\thesisdecision

% [自动生成] 常规评阅人名单：\thesisreviewers{总评阅人数}{常规评阅人数}，首句为「本学位论文共接受 … 位专家评阅，其中常规评阅人 … 名。」非盲审另起一行「名单如下：」及表格
\thesisreviewers{2}{0}

% [自动生成] 独创性声明
\thesisdeclarations

\end{document}